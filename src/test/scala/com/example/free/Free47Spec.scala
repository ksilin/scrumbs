...
class Free47Spec extends FreeSpec with MustMatchers {
  ...
  "Applications as coproduct of free algrebras" - {
    ...
    type Application[A] = EitherK[Interact, DataOp, A]
    ...
    class DataOps[F[_]](implicit I: InjectK[DataOp, F]) {
      def addCat(a: String): Free[F, String]  = Free.inject[DataOp, F](AddCat(a))
      def getAllCats(): Free[F, List[String]] = Free.inject[DataOp, F](GetAllCats())
    }

    object DataOps {
      implicit def dataOps[F[_]](implicit I: InjectK[DataOp, F]): DataOps[F] = new DataOps[F]
    }
    ...
    class LogOps[F[_]](implicit I: InjectK[LogOp, F]) {
      def error(msg: String) = Free.inject(Error(msg))
      def debug(msg: String) = Free.inject(Debug(msg))
    }

    object LogOps {
      implicit def LogOps[F[_]](implicit i: InjectK[LogOp, F]) = new LogOps[F]
    }
    ...
    "and a third algebra is factored in" in {
      ...
      type C01[A]          = EitherK[Interact, DataOp, A]
      type Application2[A] = EitherK[LogOp, C01, A]
